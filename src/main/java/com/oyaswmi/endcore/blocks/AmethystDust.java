...
import static com.oyaswmi.endcore.blocks.CoreBlocks.AMETHYST_DUST;
import static com.oyaswmi.endcore.blocks.CoreBlocks.CARVED_RUNE;

// look man imma be real idk what half this shit does but everything breaks when i so much as remove code that isn't being used so.
public class AmethystDust extends Block {
    ...
    private BlockState getPlacementState(BlockView world, BlockState state, BlockPos pos) {
        boolean bl7;
        boolean bl = AmethystDust.isNotConnected(state);
        state = this.getDefaultWireState(world, (BlockState)this.getDefaultState(), pos);
        if (bl && AmethystDust.isNotConnected(state)) {
            return state;
        }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
        ...
        if (wireConnection.isConnected() == ((WireConnection)state.get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction))).isConnected() && !AmethystDust.isFullyConnected(state)) {
        ...
        return this.getPlacementState(world, (BlockState)((BlockState)this.dotState.with(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction), wireConnection)), pos);
    ...
    public void prepare(BlockState state, WorldAccess world, BlockPos pos, int flags, int maxUpdateDepth) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            if (!blockState.isOf(Blocks.OBSERVER)) {
                ...
                AmethystDust.replace(blockState, blockState2, world, mutable, flags, maxUpdateDepth);
            }
            ...
            AmethystDust.replace(blockState3, blockState4, world, mutable, flags, maxUpdateDepth);
        }
    }
    ...
    private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction, boolean bl) {
        ...
        if (bl && (bl2 = this.canRunOnTop(world, blockPos, blockState)) && AmethystDust.connectsTo(world.getBlockState(blockPos.up()))) {
            if (blockState.isSideSolidFullSquare(world, blockPos, direction.getOpposite())) {
                return WireConnection.UP;
            }
            return WireConnection.SIDE;
        }
        if (AmethystDust.connectsTo(blockState, direction) || !blockState.isSolidBlock(world, blockPos) && AmethystDust.connectsTo(world.getBlockState(blockPos.down()))) {
            return WireConnection.SIDE;
        }
    ...
    public void neighborUpdate(BlockState state, World world, BlockPos pos, Block block, BlockPos fromPos, boolean notify) {
        ...
        if (state.canPlaceAt(world, pos)) {
           // this.update(world, pos, state);
            ...
            AmethystDust.dropStacks(state, world, pos);
    ...
    protected static boolean connectsTo(BlockState state) {
        return AmethystDust.connectsTo(state, null);
    }

    protected static boolean connectsTo(BlockState state, @Nullable Direction dir) {
        if (state.isOf(AMETHYST_DUST) || state.isOf(CARVED_RUNE)) {
            return true;
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (AmethystDust.isFullyConnected(state) || AmethystDust.isNotConnected(state)) {
            BlockState blockState = AmethystDust.isFullyConnected(state) ? this.getDefaultState() : this.dotState;
